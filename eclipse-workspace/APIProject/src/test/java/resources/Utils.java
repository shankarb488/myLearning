...
public class Utils {
	public static RequestSpecification addPlaceSpecReq;
	public static ResponseSpecification addPlaceSpecRes;

	public RequestSpecification requestSpecification() throws IOException {

		if (addPlaceSpecReq == null) {
			PrintStream log = new PrintStream(new FileOutputStream("Logging.txt"));

			addPlaceSpecReq = new RequestSpecBuilder().setBaseUri(getGlobalValue("baseUrl"))
					.addQueryParam("key", "qaclick123").addFilter(RequestLoggingFilter.logRequestTo(log))
					.addFilter(ResponseLoggingFilter.logResponseTo(log)).addHeader("Content-Type", "application/json")
					.build();

			return addPlaceSpecReq;
